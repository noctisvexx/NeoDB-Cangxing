...
- **应用启动慢**：优先使用**安装版**（`藏星 Setup.exe`），安装后从开始菜单/桌面启动，实测约 1~2 秒就绪；绿色版（单文件 exe）每次启动都要先解压整个应用，启动明显更慢（可能 20~30 秒），仅建议临时分享用
- **杀毒软件拖慢首次启动**：首次运行新版本时 Windows Defender 等会扫描 exe 与资源文件，可把安装目录加入杀毒软件排除列表（或直接信任此应用）
